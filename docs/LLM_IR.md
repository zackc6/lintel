...
|---|---|---|
| **Design philosophy** | Selector / Translator / Generator | SLA `propose` is **Selector** (allowlisted schedule). Translator (`paste_cuda`, free LLVM/Triton rewrite) is **illegal on SLA**. Generator-lite = the **inspectable schedule JSON**, then Triton runs. Magellan-class Generator (new passes) is job (b) — **out**. |
...
| **Abstraction** | NL · PL · LLVM IR/ASM; intra-level vs cross-level (compile/decompile) | Lintel IR sits **above** all three. NL = commentary only. PL payload year-1 = Triton **schedule fields**, not CUDA source. LLVM IR/PTX = classical **L0/L5**. Cross-level “LLM is the compiler” (C→x86, decompile) is **survey M3** — out. |
...
1. **Verify at the same stratum as `propose`.** A schedule proposal is gated by `adapter_gate` `{where: smem|occupancy}` then golden/numerical on the cubin. Alive2-on-LLVM is the wrong oracle for that payload. If lab later allows Translator of Triton **source**, that is a new gate set, not a reuse of smem checks.

2. **The grammar constraint is the JSON schema**, not xgrammar over CUDA. Typed `schedule` already forces a legal Selector action. Constrained decoding of kernel text is a Translator tax we do not pay in year 1.

3. **Interpretability is the admit record + schedule**, not Chain-of-Thought. The paper’s best Generator pattern is “LLM emits a script a compiler then runs.” That script is `adapter-proposal.v0`, not chat.
...
5. **Self-improving = T5-lite, not a new `opt`.** Recurring `{where}` → tighter schedule bounds or a new oracle plugin (test-gated). Do not staff “Translator finds a trick → we emit a new LLVM pass.”
...
| Selector as the safe default (C3-B) | Translator as the demo (KernelBench theater) |
...
```text
 ...
 Triton schedule JSON   adapter-proposal.v0         ← year-1 LLM-facing L4 (Selector)
 Cake IR / Tile / HIP   later adapter_id            ← still not Lintel IR
 LLVM IR / PTX          classical L0 / L5           ← oracles after lower; no propose
```

Year-1 executable is still [POC.md](POC.md) + [DATA_PLANE.md](DATA_PLANE.md). This survey does not add a third IR.
